...
#ifndef ZMQ_CLONE_SERVERS_ZHELPERS_HPP
#define ZMQ_CLONE_SERVERS_ZHELPERS_HPP
...
inline static char *
s_recv(void *socket, const int flags = 0) {
	zmq_msg_t message;
	...
	const auto string = static_cast<char *>(malloc(size + 1));
...
{
	zmq::message_t message;
	const zmq::recv_result_t rc = socket.recv(message, flags);
...
s_send(void *socket, const char *string, int flags = 0) {
	...
	const int rc = zmq_msg_send(&message, socket, flags);
	assert(-1 != rc);
	zmq_msg_close(&message);
	return (rc);
}
...
s_send (zmq::socket_t & socket, const std::string & string, zmq::send_flags flags = zmq::send_flags::none) {
	...
	const zmq::recv_result_t rc = socket.send (message, flags);
    return static_cast<bool>(rc);
}
...
s_sendmore(void *socket, const char *string) {
	...
	const int rc = zmq_msg_send(&message, socket, ZMQ_SNDMORE);
	assert(-1 != rc);
	zmq_msg_close(&message);
	return (rc);
}
...
s_sendmore (zmq::socket_t & socket, const std::string & string) {
	...
	const zmq::send_result_t rc = socket.send (message, zmq::send_flags::sndmore);
    return static_cast<bool>(rc);
}
...
#endif //ZMQ_CLONE_SERVERS_ZHELPERS_HPP